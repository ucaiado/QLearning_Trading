...
class BloombergMatching(OrderMatching):
    ...
    def translate_row(self, idx, row, my_book):
        '''
    ...
    def next(self):
        ...
        if int(self.idx) > self.max_nfiles:
            raise StopIteration
        # if it is the first line of the file, open it and cerate a new order book
        if self.i_nrow == 0:
        ...
        try:
            ...
            l_msg = self.translate_row(self_i_nrow, row, sefl.my_book)
            self.i_nrow += 1
            return l_msg
        except StopIteration:
            self.i_nrow = 0
            self.idx += 1
            print self.my_book.get_n_top_prices(100)
            raise StopIteration
